...
def _context() -> ResearchContext:
    ...
    return ResearchContext(
        ...
        facts=FactView(company_id="synthetic:portfolio", decision_ts=timestamp, values={}, evidence_refs_by_fact={}, reporting_period=ReportingPeriod(), accounting_scope=AccountingScope()),
    )


def test_module_outputs_only_typed_thesis_portfolio_artifact() -> None:
    thesis = Thesis(
        thesis_key="thesis:a", company_id="synthetic:portfolio", title="A",
        statement="A", mechanism="A", anti_thesis="not A", status="active",
        falsifier_statements=("break",), next_check_date=date(2026, 12, 1),
        confidence=0.9,
