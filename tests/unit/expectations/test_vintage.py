...
def v(as_of, np):
    return ConsensusVintage(
        company_id="300034.SZ",
        as_of=datetime.fromisoformat(as_of).replace(tzinfo=timezone.utc),
...
def test_snapshot_uses_latest_vintage_known_at_decision_time():
    ...
    s.add(v("2026-05-01", 2.5))
    s.add(v("2026-08-26", 2.1))
...
def test_snapshot_can_select_internal_vs_sell_side_vintage():
    ...
    s.add(
        ConsensusVintage(
            ...
            forecast_period="2026FY",
            net_profit=2.2,
            expectation_type="internal",
        )
    )
    assert (
        s.snapshot(
            "X", datetime(2026, 5, 2, tzinfo=timezone.utc), expectation_type="internal"
        ).net_profit
        == 2.2
    )
