...
def test_roic_incremental_roic_and_iwcr():
    r = CapitalEfficiencyEngine().calculate(
        {
            "nopat": 12,
            "invested_capital_begin": 90,
            "invested_capital_end": 110,
            "nopat_prev": 9,
            "invested_capital_prev": 90,
            "delta_nwc": 60,
            "delta_revenue": 100,
    ...
    assert r.roic == pytest.approx(0.12)
    assert r.incremental_roic == pytest.approx(0.15)
    assert r.iwcr == pytest.approx(0.60)
    ...
    c = CapitalEfficiencyEngine().growth_quality_components(
        {
            ...
            "roic": 0.10,
            "cash_conversion": 0.8,
            "incremental_nwc_efficiency": 0.7,
            "leverage_deterioration": 0.1,
            "dilution": 0.0,
        }
    )
    assert set(c) == {
