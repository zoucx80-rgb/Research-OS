...
REFERENCE_ROOT = (
    Path(__file__).parents[2] / "fixtures" / "historical_replay" / "v1_5_12"
)
...
def test_v1_5_12_reference_files_match_the_verified_historical_manifest() -> None:
    ...
    assert manifest["verification_mode"] == (
        "detached_worktree_with_explicit_historical_src"
    )
...
def test_runtime_reference_retains_kpi_missingness_lineage_and_completion() -> None:
    manufacturer = _json(
        REFERENCE_ROOT / "runtime_reference" / "anonymous_manufacturer.json"
    )
    distributor = _json(
        REFERENCE_ROOT / "runtime_reference" / "anonymous_distributor.json"
    )
    ...
    for payload in (manufacturer, distributor):
        ...
        assert all(
            item["evidence_ids"] for item in metrics.values() if item["status"] == "valid"
        )
