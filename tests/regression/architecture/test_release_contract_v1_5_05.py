...
def test_public_v1_5_05_version_and_reporting_fingerprints_are_consistent():
    ...
    assert RESEARCH_OS_VERSION == "1.5.5"
    assert research_os.__version__ == "1.5.5"
    assert project["project"]["version"] == "1.5.5"
    assert metadata["research_os_version"] == "1.5.5"
    ...
    assert metadata["module_versions"]["report_composer"] == "1.0.0"
    assert ResearchViewPresenter.version == "professional-research-view@1.3.0"
    assert ResearchReportComposer.version == "research-report-composer@1.0.0"
