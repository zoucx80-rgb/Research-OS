...
class DecisionSummaryPresenter:
    ...
    _SEMANTICS: dict[str, dict[str, tuple[str, str]]] = {
        ...
        "business_model": {
            ...
            "consumer": ("消费与零售业务", "公司主要面向消费者提供品牌、零售或消费产品与服务创造经济价值。"),
        ...
        "thesis_state": {
            ...
            "UNRESOLVED": ("投资逻辑等待确认", "当前证据方向混合且缺少可验证的既有方向性逻辑，因此保持未决并等待后续确认。"),
            "UNKNOWN": ("投资逻辑状态尚不明确", "当前没有足够结构化证据确定投资逻辑状态。"),
        },
        "fundamental_state": {
            "IMPROVING": ("基本面改善", "当前状态值为基本面改善；请结合状态来源确认这是系统推导还是分析师输入。"),
            "STABLE": ("基本面稳定", "当前状态值为基本面稳定；请结合状态来源确认这是系统推导还是分析师输入。"),
            "DETERIORATING": ("基本面恶化", "当前状态值为基本面恶化；请结合状态来源确认这是系统推导还是分析师输入。"),
            "UNCERTAIN": ("基本面方向尚不确定", "当前状态值尚不支持明确方向；请结合状态来源与证据查看。"),
        },
        "expectation_state": {
            "UNDER_EXPECTED": ("低于市场预期", "当前状态值为低于市场预期；请结合状态来源和市场预期证据查看。"),
            "IN_LINE": ("大致符合市场预期", "当前状态值为大致符合市场预期；请结合状态来源和市场预期证据查看。"),
            "OVER_EXPECTED": ("高于市场预期", "当前状态值为高于市场预期；请结合状态来源和市场预期证据查看。"),
            "MIXED": ("市场预期信号混合", "当前状态值为混合；请结合状态来源和市场预期证据查看。"),
            "UNKNOWN": ("市场预期证据不足", "当前缺少可审计的 PIT 市场预期证据，因此不形成方向性市场预期结论。"),
        },
        "valuation_state": {
            "CHEAP": ("估值偏低", "当前状态值为估值偏低；请结合状态来源、估值模型与执行血缘查看。"),
            "FAIR": ("估值大致合理", "当前状态值为估值大致合理；请结合状态来源、估值模型与执行血缘查看。"),
            "EXPENSIVE": ("估值偏高", "当前状态值为估值偏高；请结合状态来源、估值模型与执行血缘查看。"),
            "UNRELIABLE": ("估值结果可靠性不足", "当前状态值为估值可靠性不足；请结合状态来源与估值执行证据查看。"),
        },
        "decision_state": {
        ...
        "reason": {
            ...
            "DEBT_FUNDS_NWC": ("新增债务主要支持营运资金", "营运资金扩张主要依赖新增债务融资支持，需要关注融资成本与偿债压力。"),
            "NEGATIVE_OCF": ("经营现金流为负", "经营活动产生的现金流为负，需要核查应收、存货及经营性负债变化。"),
            "EQUITY_DILUTION": ("存在股权融资稀释", "增长或营运资金需求部分依赖新增股权资本，需关注股东稀释。"),
            "MATERIAL_FACTORING_EXPOSURE": ("保理或终止确认应收暴露较大", "已披露保理或终止确认应收相对期末应收具有重要性；该暴露不自动等同于债务。"),
        },
    }
    ...
    def present(
        ...
        return HumanReadableDecisionSummary(
            ...
            expectation_evidence_status=self.semantic(summary.expectation_evidence_status, category="module_status"),
            valuation_execution_status=self.semantic(summary.valuation_execution_status, category="module_status"),
