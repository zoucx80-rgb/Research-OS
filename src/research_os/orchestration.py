    ...
    def complete_run(self,req:ResearchRunRequest)->ResearchRun:
        ...
        if safety is not None:
            ...
            if safety.financial_observations:
                ...
                if consistency.status=="FAIL": financial_result.status="FAIL"
        ...
        statuses["KPI Pack"]="PASS" if packs else "FAIL"
        metrics:list[MetricResult]=[]
        ...
        return ResearchRun(
            company_id=req.company_id,decision_ts=req.decision_ts,profile=profile,pack_ids=[p.pack_id for p in packs],
            metrics=metrics,capital_efficiency=capital_efficiency,funding_loop=funding_loop,drivers=drivers,theses=theses,
            claims=claims,expectations=expectation,valuation=valuation,decision=decision,snapshot=snapshot,
            validation_statuses=statuses,completion=completion,
        )
