...
def _repository_identity(
    ...
    *,
    ...
    preflight = RepositoryPreflightEvidence(
        ...
        head_sha=frozen_sha,
        head_commit_message=_git(repository_root, "show", "-s", "--format=%s", frozen_sha),
        agents_blob_sha=_git(repository_root, "rev-parse", f"{frozen_sha}:AGENTS.md"),
        research_prompt_blob_sha=_git(
            repository_root,
            "rev-parse",
...
def _case_evidence(
    ...
    *,
    ...
    for raw in payload.get("evidence", []):
        ...
        publish_ts = datetime.fromisoformat(str(record["publish_ts"]).replace("Z", "+00:00"))
...
def render_active_case(
    *,
    ...
    decision_ts = datetime.fromisoformat(str(payload["decision_ts"]).replace("Z", "+00:00"))
    ...
    context = ResearchContext(
        run_id=str(payload.get("run_id") or f"field:active:{case_id}"),
        company=company,
        decision_ts=decision_ts,
        baseline=baseline,
