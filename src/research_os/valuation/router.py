    ...
    def route(self,context:ValuationContext)->ValuationRoutingResult:
        ...
        for name,score in scores.items():
            ...
            if context.business_model=="distributor" and name=="dcf" and context.models[name].cash_flow_visibility<.5:
                adjusted*=.5
            if (
                context.business_model=="distributor"
                and name=="pe"
                and context.funding_state=="debt_funded"
                and "NEGATIVE_OCF" in context.funding_reason_codes
            ):
                adjusted*=.25
                reasons.append("CASH_FUNDING_RISK_PE_PENALTY")
            ...
            routed[name]=RoutedModel(model_name=name,score=adjusted,status=status,reason_codes=reasons)
