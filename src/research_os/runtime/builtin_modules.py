...
def _safety(legacy_request: Any) -> Any | None:
    return getattr(legacy_request, "safety", None) if legacy_request is not None else None


def _status_artifact(module_id: str, status: str, key: str, value: Any, *, evidence_ids=None, diagnostics=None):
    return ModuleResult(
        module_id=module_id,
        status=status,
        artifacts={key: value},
...
class RepositoryPreflightModule:
    ...
    def __init__(self, validator: PreflightValidator | None = None, legacy_request: Any = None):
        self.validator = validator or PreflightValidator()
        self.legacy_request = legacy_request

    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        safety = _safety(self.legacy_request)
        evidence = getattr(safety, "preflight", None) if safety is not None else None
        if evidence is None:
            return _status_artifact(self.spec.module_id, "INSUFFICIENT_EVIDENCE", "validation.repository_preflight", None)
        try:
            result = self.validator.validate(evidence)
        except ValueError as exc:
            return _status_artifact(self.spec.module_id, "FAIL", "validation.repository_preflight", None, diagnostics=[str(exc)])
        return _status_artifact(self.spec.module_id, "PASS", "validation.repository_preflight", result)
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        if not evidence:
            return ModuleResult(
                ...
                artifacts={"evidence.pit": [], "validation.lineage": {"status": "INSUFFICIENT_EVIDENCE"}},
...
class FinancialSanityModule:
    ...
    def __init__(self, validator: FinancialSanityValidator | None = None, legacy_request: Any = None):
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        safety = _safety(self.legacy_request)
        if safety is None:
            return _status_artifact(self.spec.module_id, "INSUFFICIENT_EVIDENCE", "validation.financial", None)
        result = self.validator.validate_fact_mapping(
            context.facts.as_mapping(),
            unit=getattr(safety, "financial_unit", "元"),
        )
        observations = getattr(safety, "financial_observations", []) or []
        if observations:
            consistency = self.validator.check_consistency(observations)
            result.errors.extend(consistency.errors)
            if consistency.status == "FAIL":
                result.status = "FAIL"
        status = "FAIL" if result.status == "FAIL" else "PASS"
        return _status_artifact(self.spec.module_id, status, "validation.financial", result, diagnostics=result.errors)
...
class BusinessModelModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        if not evidence:
            return _status_artifact(self.spec.module_id, "INSUFFICIENT_EVIDENCE", "business_model.profile", None)
...
class StrategyResolutionModule:
    ...
    def __init__(self, *, registry: PluginRegistry, resolver: StrategyResolver | None = None):
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        if profile is None:
            return _status_artifact(self.spec.module_id, "INSUFFICIENT_EVIDENCE", "strategy.resolution", None)
        resolution = self.resolver.resolve(profile, context, self.registry)
        primary_gap = any(
            gap.gap_type == "industry_strategy" and gap.business_model == profile.primary_model
            for gap in resolution.coverage_gaps
        )
        status = "INSUFFICIENT_EVIDENCE" if primary_gap or not resolution.industry_plugins else "PASS"
        return _status_artifact(
            self.spec.module_id,
            status,
            "strategy.resolution",
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        if profile is None or resolution is None or not resolution.industry_plugins:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"kpi.metrics": [], "kpi.pack_ids": [], "validation.kpi": {"status": "INSUFFICIENT_EVIDENCE"}},
        ...
        for resolved in resolution.industry_plugins:
            ...
            pack_id = getattr(pack, "pack_id", resolved.plugin_id.removeprefix("industry:"))
            ...
            for module in plugin.modules():
                ...
                for metric in result.artifacts.get("kpi.metrics", []):
                    ...
                    metrics.append(metric.model_copy(update={"evidence_ids": ids}) if ids else metric)
...
class CapitalEfficiencyModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        valid_kpi = any(
            metric.status == "valid" and metric.metric_id in {"roic", "incremental_roic", "incremental_nwc_intensity"}
            for metric in metrics
        )
        status = "PASS" if valid_kpi or any(value is not None for value in (result.roic, result.incremental_roic, result.iwcr)) else "INSUFFICIENT_EVIDENCE"
        return ModuleResult(
            module_id=self.spec.module_id,
            status=status,
            artifacts={"capital.efficiency": result, "validation.capital": {"status": status}},
        )
...
class FundingLoopModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        status = "INSUFFICIENT_EVIDENCE" if result.funding_state == "unknown" else "PASS"
        return ModuleResult(
            module_id=self.spec.module_id,
            status=status,
            artifacts={"capital.funding_loop": result, "validation.funding": {"status": status}},
        )
...
class DriverThesisModule:
    spec = ModuleSpec(
        ...
        provides={"drivers.graph", "thesis.items", "claims.items", "validation.thesis"},
    )

    def __init__(self, theses: ThesisService | None = None, ledger: EvidenceLedger | None = None):
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        ...
        if not evidence:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"drivers.graph": None, "thesis.items": [], "claims.items": [], "validation.thesis": {"status": "INSUFFICIENT_EVIDENCE"}},
            )
        drivers = DriverGraph.build(context.company.company_id, pack_ids, evidence)
        theses = self.theses.evaluate(context.company.company_id, evidence, drivers)
        claims = []
        for thesis in theses:
            ...
            claim = Claim(
                ...
                claim_type="thesis",
                confidence_grade="D",
                evidence_ids=thesis.supporting_evidence or [item.evidence_id for item in evidence],
        ...
        return ModuleResult(
            module_id=self.spec.module_id,
            status=status,
            artifacts={"drivers.graph": drivers, "thesis.items": theses, "claims.items": claims, "validation.thesis": {"status": status}},
            evidence_ids=[item.evidence_id for item in evidence],
        )
...
class ExpectationModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        vintage = getattr(self.legacy_request, "expectation_vintage", None) if self.legacy_request is not None else None
        if vintage is None:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"expectation.snapshot": None, "validation.expectation": {"status": "INSUFFICIENT_EVIDENCE"}},
            )
        self.service.add(vintage)
        snapshot = self.service.snapshot(context.company.company_id, context.decision_ts, expectation_type=vintage.expectation_type)
        safety = _safety(self.legacy_request)
        status = "INSUFFICIENT_EVIDENCE"
        diagnostics: list[str] = []
        if safety is not None:
            assessment = self.validator.assess(
                conclusion=getattr(safety, "expectation_conclusion", None),
                evidence=getattr(safety, "expectation_evidence", None),
        ...
        return ModuleResult(
            module_id=self.spec.module_id,
            status=status,
            artifacts={"expectation.snapshot": snapshot, "validation.expectation": {"status": status}},
...
class ForecastDisciplineModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        return _status_artifact(
            ...
            "NOT_APPLICABLE",
            "forecast.discipline",
            {"status": "NOT_APPLICABLE", "reason": "no promoted forecast methodology"},
        )
...
class ValuationModule:
    ...
    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        profile = state.get("business_model.profile")
        models = getattr(self.legacy_request, "valuation_models", None) if self.legacy_request is not None else None
        if profile is None or models is None:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"valuation.routing": None, "validation.valuation": {"status": "INSUFFICIENT_EVIDENCE"}},
            )
        routing = self.router.route(ValuationContext(business_model=profile.primary_model, models=models))
        status = "PASS" if routing.primary_models else "INSUFFICIENT_EVIDENCE"
        safety = _safety(self.legacy_request)
        execution = getattr(safety, "valuation_execution", None) if safety is not None else None
        ...
        return ModuleResult(
            module_id=self.spec.module_id,
            status=status,
            artifacts={"valuation.routing": routing, "validation.valuation": {"status": status}},
        )
...
class DecisionModule:
    spec = ModuleSpec(
        ...
        module_version="1.0.0",
        requires={"evidence.pit", "thesis.items", "claims.items", "expectation.snapshot", "valuation.routing"},
        provides={"decision.record", "validation.decision"},
    )

    def __init__(self, engine: DecisionEngine | None = None, legacy_request: Any = None):
    ...
    def _confidence(evidence) -> float:
        if not evidence:
            return 0.0
        return sum(_GRADE_SCORE.get(item.confidence_grade.value, 0.0) for item in evidence) / len(evidence)

    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        if self.legacy_request is None:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"decision.record": None, "validation.decision": {"status": "INSUFFICIENT_EVIDENCE"}},
            )
        ...
        if thesis_state not in {"STRENGTHENING", "ACTIVE", "WEAKENING", "FALSIFIED"}:
            thesis_state = "ACTIVE"
        versions = getattr(self.legacy_request, "versions", {}) or {}
        decision = self.engine.evaluate(DecisionContext(
            company_id=context.company.company_id,
            fundamental_state=self.legacy_request.fundamental_state,
            valuation_state=self.legacy_request.valuation_state,
            expectation_state=self.legacy_request.expectation_state,
            thesis_state=thesis_state,
            evidence_confidence=self._confidence(evidence),
            evidence_ids=[item.evidence_id for item in evidence],
            claim_ids=[claim.claim_id for claim in claims],
            decision_ts=context.decision_ts,
            research_os_version=versions.get("research_os_version", context.baseline.research_os_version),
        ))
        validate_decision_state(decision.state)
        return ModuleResult(
            module_id=self.spec.module_id,
            status="PASS",
            artifacts={"decision.record": decision, "validation.decision": {"status": "PASS"}},
            evidence_ids=[item.evidence_id for item in evidence],
        )
...
class TemporalModule:
    ...
    def __init__(self, validator: NextVerificationEventValidator | None = None, legacy_request: Any = None):
        self.validator = validator or NextVerificationEventValidator()
        self.legacy_request = legacy_request

    def run(self, context: ResearchContext, state: ResearchStateView) -> ModuleResult:
        safety = _safety(self.legacy_request)
        event = getattr(safety, "next_verification_event", None) if safety is not None else None
        if event is None:
            return ModuleResult(
                module_id=self.spec.module_id,
                status="INSUFFICIENT_EVIDENCE",
                artifacts={"temporal.result": None, "validation.temporal": {"status": "INSUFFICIENT_EVIDENCE"}},
            )
        used_ids = [item.evidence_id for item in context.evidence.as_of(context.decision_ts)]
        result = self.validator.validate(event, reference_time=context.decision_ts, used_evidence_ids=used_ids)
        return ModuleResult(
            module_id=self.spec.module_id,
            status=result.status,
            artifacts={"temporal.result": result, "validation.temporal": {"status": result.status}},
            evidence_ids=used_ids,
        )


def build_builtin_modules(*, registry: PluginRegistry, legacy_request: Any = None):
    capital = CapitalEfficiencyEngine()
    return [
        RepositoryPreflightModule(legacy_request=legacy_request),
        PITLineageModule(),
        FinancialSanityModule(legacy_request=legacy_request),
        BusinessModelModule(),
        ...
        DriverThesisModule(),
        ExpectationModule(legacy_request=legacy_request),
        ForecastDisciplineModule(),
        ValuationModule(legacy_request=legacy_request),
        DecisionModule(legacy_request=legacy_request),
        TemporalModule(legacy_request=legacy_request),
    ]
