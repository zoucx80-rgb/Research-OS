...
- [x] 写 RED 测试：Schema/Codec/Hash 版本必填，UTC 时间、run/company/baseline/version/fingerprint 必填。
- [x] 写 RED 测试：NaN、Infinity、无 Schema Artifact 和可变 payload 被拒绝。
- [x] 实现冻结的 `ResearchSnapshotV2`、`ResearchSnapshotPayloadV2`、`ArtifactFingerprint`、`ResearchSnapshotDescriptor`。
...
- [x] 写 RED 测试：对象 key 顺序不影响 bytes/hash；相同研究输入不同 run ID 的 research digest 相同，完整性 digest 可不同。
- [x] 写 RED 测试：Datetime、Decimal、Enum、Pydantic Model 有唯一规范形式。
- [x] 写 RED 测试：未知对象类型、非字符串 key、NaN/Infinity 失败关闭。
- [x] 写 RED 测试：未知 Artifact Schema/type 不解码，禁止从快照动态 import Python 路径。
- [x] 复用 RFC 8785/JCS 库，不用 `default=str` 或自行发明浮点规则。
- [x] 用 Hypothesis 验证确定性与 round-trip normalization。
...
- [x] 定义 Snapshot/Run/Evidence Repository Protocol 和分页 Query 类型。
- [x] 写共享 Repository Contract Tests。
- [x] 实现 InMemory Adapter，只用于测试，复制输入并返回不可变值。
- [x] Append-only：重复 Snapshot ID、重复 Run ID、替换版本均失败。
...
- [x] 写 RED migration test：从 1.5.12 head 升级后存在 `research_run`、`research_snapshot`、`artifact_index`。
- [x] 写 RED test：升级不改写旧 `evidence` 行。
- [x] 实现主键、唯一约束、外键和 PIT/decision_ts 索引。
- [x] Downgrade 仅删除新增表/索引，不删除旧数据。
- [x] SQLite 作为契约后端；对生产数据库差异保持 SQLAlchemy 可移植性。
...
- [x] 写 RED：进程重启后 Snapshot 可读取并验证。
- [x] 写 RED：Run/Snapshot 同事务提交，任一失败时整体 rollback。
- [x] 写 RED：latest-as-of 在数据库侧按 `company_id + decision_ts` 选择具体 publish_ts/revision，返回不可变 `EvidenceRef`；未来 revision 不得进入历史 FactView。
- [x] 使用窗口函数或相关子查询，不在 Python 中读取全部 revision 后归并。
- [x] Mapper 保留 `company_id`、period、scope、unit、revision、UTC 时间、raw/normalized value、`comparison_basis`、`metric_kind`、source locator、lineage 和 content hash；旧行缺失值保持 NULL/UNKNOWN，不推断或重写。
...
- [x] 写 RED：Application 完成后写入 Run + Snapshot + Artifact Index。
- [x] 写 RED：research digest 覆盖研究语义投影；integrity digest 覆盖完整 envelope、versions、component/artifact fingerprints，且不递归包含自身或 SnapshotDescriptor。
- [x] 写 RED：篡改数据库 payload 或版本后 `verify()` 返回明确失败原因。
- [x] SnapshotWriter 不改变 Artifact 值，只序列化最终 Snapshot。
...
- [x] 写 RED：not found、无效 cursor、超限 limit、PIT 上界。
- [x] Query Service 只依赖 Repository/Projector Port。
- [x] Artifact Query 必须返回 Schema、Provider 和 Evidence metadata。
...
- [x] 写 RED：所有 `/api/v1` endpoints、分页、PIT 参数和 response_model。
- [x] 写 RED：错误为 `application/problem+json`，包含稳定 type/status/request_id。
- [x] 写 RED：每个响应含 `X-Request-ID`，允许上游合法 ID，拒绝控制字符。
- [x] 写 RED：HTTP API 版本不等于 Core API 版本。
- [x] 实现 middleware、exception handlers 和 typed DTO。
- [x] 删除正式 `ResearchReadStore` 默认路径；内存 Query Adapter 放到 tests helper。

### Task 9：M2 出口门禁

- [x] `alembic upgrade head -> downgrade -1 -> upgrade head` 通过。
- [x] Snapshot restart/tamper/append-only tests 通过。
- [x] Evidence latest-as-of SQL query tests 通过。
- [x] API integration 与 OpenAPI golden contract 通过。
- [x] 确认 API/Persistence 未被 Domain 导入。
